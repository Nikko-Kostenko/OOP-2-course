...
namespace LabCalc1
{
    ...
    class Rpn
    {
        private const string EROR_TOO_MANY_OPERATORS = "забагато операторів підряд";
         ...
         static private double GetCellValue(string input, int i, DataGridView Table)
         {
             ...
             for (int i = 0; i < input.Length; i++)
             {
                 ...
                 if (IsOneSymbolOperator(input[i]) || IsThreeSymbolOperator(input, i))
                 {
                     ...
                     else if (UnaryMinus(input, i))
                     {
                         ...
                         {
                             if (GetCellValue(input, i, Table) >= 0)
                             {
                                 output += "0 ";
                             ...
                             {
                                output += Math.Abs(GetCellValue(input, i, Table)) + " ";
